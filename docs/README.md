...
- プロジェクト詳細：
  ...
  - APIの新規作成、修正、Rspecによる単体テスト実装、フロントエンド修正を行った
  - タスクとフランジの紐づけ機能およびCSVでのインポートAPIの実装、N+1改善やモーダルのフロントエンド制御の修正など
  - 前述の進捗システムとのシステム連携
- 使用されていた技術：
  - Rails 7.0.3
  - Vue.js 2系
...
- 気づき・苦労したところ：
  ...
  - 参画当初はGitブランチ運用があまりよくなく新しく作り直し改善した
...
- プロジェクト詳細：
  ...
  - APIの新規作成、修正、Rspecによる単体テスト実装を行った
  - ユーザーの一括承認APIの新規実装、ガントチャートAPIのレスポンス修正など
- 使用されていた技術：
  - Rails 7.0.3
  - Vue.js 2系
  - MySQL 8 系
...
- 気づき・苦労したところ：
  ...
  - Rspecでのテスト、Rubocopでのlint修正をきっちり対応することで自分の実装に自身が持てるようになった。
  - 単体テストでも仕様を理解していないと作成することも難しいと感じた
...
- プロジェクト詳細：
  ...
  - 簡単なAPIの修正を行った。例えばメール送信対象のユーザー取得クエリの修正やTODOデータのサニタイズなど
- 使用されていた技術：
  - Rails 7.0.3
  - Vue.js 2系
  - MySQL 8 系
  - Docker
- 気づき・苦労したところ：
  - 今まではMPAでの開発がほとんどでAPIのデバッグやフロントエンドとのつなぎこみに苦労した
  - RailsとLaravelとのギャップに苦しんだ。似て非なるものでディレクトリ構成が地味に異なっていたり、バリデーションを実行するファイルが異なったりと慣れるまで苦労した。 
